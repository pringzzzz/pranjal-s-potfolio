...
# Extract the template JSON - keep the raw JSON without parsing
...
# Check if the JSON is properly quoted (escaped)
if not json_str.startswith('"') or not json_str.endswith('"'):
    print("Error: JSON is not properly escaped")
...
# 1. Replace overflow-x: html, body rule already added in CSS

# 2. Update the responsive CSS with tablet breakpoint and new mobile requirements
new_responsive_css = """<style id="__responsive">
...
# 3. Add credibility strip after hero buttons (find the Say hello button location)
...
# 4. Add max-width wrapper to hero section to prevent glow overflow
...
# Re-escape the JSON
escaped_json = json.dumps(template_data)

# Update the HTML file
new_html = html_content.replace(json_str, escaped_json)
...
with open('/Users/pranjalbharti/Documents/GitHub/pranjal-s-potfolio/index.html', 'r') as f:
    ...
    final_data = json.loads(re.search(r'<script type="__bundler/template">([\s\S]*?)</script>', final_content).group(1).strip())

    print("\nVerification:")
    changes = [
        ('overflow-x: hidden', 'Add overflow-x hidden'),
        ('@media (max-width:768px)', 'Tablet breakpoint added'),
        ('@media (max-width:480px)', 'Small mobile breakpoint added'),
        ('grid-template-columns:1fr !important', 'Grid to single column'),
        ('padding:13px 8px !important', 'Nav padding increased'),
        ('Forbes India Top 200', 'Credibility strip added'),
        ('clamp(34px,8vw,42px)', 'Headline clamp added'),
    ]

    for check, label in changes:
